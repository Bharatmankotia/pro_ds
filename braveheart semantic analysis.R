...
a<-10
braveheart<-NULL
url1<-"https://www.imdb.com/title/tt0112573/reviews?ref_="
for(i in 0:22){
  url<-read_html(as.character(paste(url1,i*a,sep="")))
  brave<-url %>%
    html_nodes(".show-more__control") %>% # used google chrome selector gadget to select the node.
    html_text() 
  braveheart<-c(braveheart,brave)
}
write.table(braveheart,file="braveheart.txt")


# Now performing semantic analysis on Braveheart Movie...
